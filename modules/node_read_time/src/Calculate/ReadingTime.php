...
class ReadingTime {
  ...
  public function calculateReadingTime() {
    ...
    if ($words_count > 1) {
      ...
      switch ($unit) {
        case 'minute':
          $reading_time = ceil($words_count / $this->wordsPerMinute) . ' minute' . (ceil($words_count / $this->wordsPerMinute) == 1 ? '' : 's');
          break;

        case 'second':
          $reading_time = $minute . ' minute' . ($minute == 1 ? '' : 's') . ', ' . $second . ' second' . ($second == 1 ? '' : 's');
          break;
